...
df_m = pd.read_excel(path, sheet_name="מדרוг", header=None)
...
with col_chart:
    with st.container(border=True):
        ...
        # עיצוב פריסה (Layout) יוקרתי ומדויק למניעת דריסה
        fig.update_layout(
            # מרווחים גדולים: r=220 נותן שטח ענק לשמות הערוצים בצד ימין. b=60 נותן מקום למקרא
            margin=dict(l=40, r=220, t=40, b=60),
            paper_bgcolor='rgba(0,0,0,0)',
            plot_bgcolor='rgba(0,0,0,0)',
            height=500,
            
            # תיקון המקרא התחתון (הוספת מרווחים קבועים כדי שלא יתעוות)
            legend=dict(
                orientation="h", 
                y=-0.15, 
                x=0.5, 
            ...
            # ציר Y ימני - מוזז החוצה באמצעות המאפיין pad כדי שלא ייגע בנקודות
            yaxis=dict(
                autorange="reversed", 
                side="right", 
                tickfont=dict(size=12, color="#0f172a"),
                pad=15 # דוחף את שמות הערוצים ימינה הרחק מהגרף
            )
        )
